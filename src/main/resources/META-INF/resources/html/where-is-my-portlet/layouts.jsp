...
<aui:form action="<%= showLayoutPortletsURL.toString() %>" method="post" name="layoutsForm">
	<aui:fieldset>
		<label><liferay-ui:message key="layouts.description" /></label>
	</aui:fieldset>

	<aui:fieldset column="true" style="padding-right:30px;">
		<aui:select label="show-private-pages" name="selectedPrivate" onChange="submitLayoutsForm();">
	...
	<aui:fieldset column="true">
		<aui:select label="site-portlets-layout" name="selectedLayout" onChange="submitLayoutsForm();">
...
<div id="<portlet:namespace/>results">
	<liferay-ui:search-container emptyResultsMessage="site-portlets-no-layouts-were-found" total="<%= portlets.size() %>">
	<liferay-ui:search-container-results results="<%= portlets %>" />

	<liferay-ui:search-container-row
		className="com.lo.whereismy.portlet.views.PortletView"
	...
	>
		...
		<%
		// Had some issue w/ tooltip class in 7.1, as of now removed it
		String goToLayoutLinkIcon = "<a title='Find out on which pages this portlet is...' class='' href='" + showPortletLayoutsFromTableURL + "'><img src='" + request.getContextPath() + "/images/application_go.png'/> " + portlet.getPortlet().getDisplayName() + " </a>";
		...
		%>
			...
			<liferay-ui:search-container-column-text name="site-portlets-portlet-id">
				...
				<portlet:renderURL var="portletPopUpURL" windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>">
					...
					<portlet:param name="layoutPlid" value="<%= String.valueOf(portlet.getLayout().getPlid()) %>" />
				...
				<a href="#" onClick="javascript:showPortletPopup<%= portlet.getPortletId() %>('<%= portletPopUpURL %>')" style="text-decoration: none;">
			...
			>
				<img
					src="<%= portlet.getPortlet().getInstanceable()?yesIcon:noIcon %>"
				/>
			</liferay-ui:search-container-column-text>
