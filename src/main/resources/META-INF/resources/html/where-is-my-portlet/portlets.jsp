...
page import="com.lo.whereismy.portlet.views.PortletView" %>
...
<div id="<portlet:namespace/>portletsFormContainer">
	<aui:form action="<%= showPortletLayoutsURL.toString() %>" method="post" name="portletsForm">
		<aui:fieldset>
			<label><liferay-ui:message key="portlets.description" /></label>
		</aui:fieldset>

		<aui:fieldset column="true" style="padding-right:30px;">
			<aui:select id="pageScope" label="show-private-pages" name="selectedPrivate" onChange="submitPortletsForm();">
				<aui:option label="Public" name="Public" selected="${selectedPrivate == false}" value="false" />
				<aui:option label="Private" name="Private" selected="${selectedPrivate == true}" value="true" />
			</aui:select>
		</aui:fieldset>

		<aui:fieldset column="true">
			<aui:select id="selectedPortlet" label="site-portlets-portlet" name="selectedPortlet" onChange="submitPortletsForm();">
				<c:forEach items="${portletNames}" var="portletName">
					<aui:option label="${portletName.value}" selected="${selectedPortlet == portletName.key}" value="${portletName.key}" />
				</c:forEach>
			</aui:select>
		</aui:fieldset>
...
<c:set value="<%= enableFilters %>" var="enableFilters"></c:set>
	...
	<div id="<portlet:namespace/>results">
		<liferay-ui:search-container emptyResultsMessage="site-portlets-no-portlets-were-found" total="${portletLayoutsSize}">
			...
			<liferay-ui:search-container-row
				className="com.lo.whereismy.portlet.views.LayoutView"
			...
			>
				...
				<%
				String goToLayoutLinkIcon = "<a title='List the portlets of this page...' href='" + showLayoutPortletsFromTableURL + "'><img src='" +
								request.getContextPath() + "/images/application_go.png'/> " +
								layoutView.getLayout().getName(themeDisplay.getLocale()) + " </a>";

				String goToLayoutUrl = WhereIsMyPortletUtil.getLayoutHREF(layoutView.getLayout(), themeDisplay, true, "Go to this portal page...", "", showBaseUrl, selectedPrivate);
				%>
				...
				>
					...
					>
						...
						<portlet:param name="layoutPlid" value="<%= String.valueOf(layoutView.getLayout().getPlid()) %>" />
						<portlet:param name="jspPage" value="/html/where-is-my-portlet/portletPopUp.jsp" />
					</portlet:renderURL>

					<div>
						<a href="#" onClick="javascript:showPortletPopup<%= layoutPortlet.getPortletId() %>('<%= portletPopUpURL %>')" style="text-decoration: none;">
				...
				<liferay-ui:search-container-column-text name="actions">
					<liferay-ui:icon-menu>
						...
						<liferay-ui:icon image="view_templates" message="goToLayout" target="_blank" url="<%= WhereIsMyPortletUtil.getLayoutUrl(themeDisplay, layoutView.getLayout(), selectedPrivate) %>" />
					</liferay-ui:icon-menu>
				</liferay-ui:search-container-column-text>
			</liferay-ui:search-container-row>
